...
def run_rustscan(ip):
    ...
    if not tool_exists("rustscan"):
        ...
        # Capture stdout + stderr combined — some RustScan versions write
        # port results to stderr, so we must not discard it with 2>/dev/null
        cmd    = f"rustscan -a {ip} --ulimit 5000 -- -Pn"
        print(f"{DIM}    $ {cmd}{RESET}")
        result = subprocess.run(cmd, shell=True, capture_output=True, text=True)
        out    = result.stdout + result.stderr   # merge both streams for parsing
        ports  = parse_ports(out)
        if not ports:
